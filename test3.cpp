...
bool init(){
    ...
    if( SDL_Init( SDL_INIT_VIDEO) < 0){
        ...
        //Create window
        window = SDL_CreateWindow( "test3", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( window == NULL )
		{
...
void draw_scene(SDL_Renderer * renderer, Line3D * lines, int n, int N){
    for(int i = 1 ; i < n ; i++){
        Line3D * curr_line = &lines[i%N];
        Line3D * prev_line = &lines[(i-1)%N];
        prev_line->project(1,1500,1);
        curr_line->project(1,1500,1);
    ...
    if( !init() ){
        ...
        if( !loadmedia() ){
            ...
            while( !quit ){
                ...
                //Draw Scene
                draw_scene(renderer, Line3DArray, 300,N);
