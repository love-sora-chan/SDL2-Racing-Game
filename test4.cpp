...
int WinMain(){
    ...
    if( !init() ){
        ...
        if( !loadmedia() ){
            ...
            int node[8]={0,100,300,500,1000,1200,1400,1700};
            double CURVE[7]={0,0.5,0,-0.7,1,0,-0.8};
            for(int i = 0 ; i < N ; i++){
                Line3D line;
                Line3DArray[i] = line;
                Line3DArray[i].z = segement_length * i;
                for(int k=0;k<7;k++){
                    if(i>=node[k]&&i<node[k+1])
                        Line3DArray[i].curve=CURVE[k];
                }
            }
            ...
            while( !quit ){
                ...
                while( SDL_PollEvent( &e ) != 0){
                    if( e.type == SDL_QUIT ){
                        ...
                        SDL_PumpEvents();
                        if(keyarr[SDL_SCANCODE_UP]&&cam->vz<=10000)
                            cam->vz+=1000;
                        if(keyarr[SDL_SCANCODE_DOWN]&&cam->vz>=-50)
                            cam->vz-=10;
                        if(keyarr[SDL_SCANCODE_LEFT])
                            cam->x-=30;
                        if(keyarr[SDL_SCANCODE_RIGHT])
                            cam->x+=30;
                        int temp = (int)(cam->z/segement_length);
                        cam->x -= Line3DArray[temp].curve*cam->vz*0.1;
